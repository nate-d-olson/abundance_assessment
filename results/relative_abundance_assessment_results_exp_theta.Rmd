---
title: "Relative Abundance Error Rate w/ experimental theta values"
author: "Nate Olson"
date: '`r Sys.Date()`'
output:
...
prop_inferred <- theta_est %>%
    ...
    dplyr::select(biosample_id, theta, t_fctr) %>%
    right_join(nb_counts) %>% right_join(pre_post_prop) %>%
    filter(t_fctr %in% c(1:5,10,15)) %>%
    ## Using experimental theta estimates to calculate expected values
    mutate(experimental_prop = post * theta + pre * (1 - theta))
...
rel_abu_error <- prop_inferred %>%
    ...
    mutate(inferred_error = abs(nb_prop - experimental_prop),
           inferred_error_rate = inferred_error/experimental_prop)
...
saveRDS(rel_abu_error, "../data/rel_abu_error.RDS")
saveRDS(rel_abu_error_summary, "../data/rel_abu_error_summary.RDS")
...
relAbuOvE <- prop_inferred %>%
    ggplot() +
    geom_smooth(aes(x = experimental_prop, y = nb_prop, color = pipe), method = "lm") +
...
relAbuOvE <- prop_inferred %>%
      ...
      mutate(pipe = forcats::fct_recode(pipe,
                                        ...
                                        QIIME = "qiime")) %>% 
    ggplot() +
      geom_smooth(aes(x = experimental_prop, y = nb_prop, color = pipe),
                  method = "lm") +
...
```{r}
ggsave("../figs/relAbuError-1.pdf",width = 170, units = "mm")
```
...
```{r}
ggsave("../figs/relAbuErrorMetrics-1.pdf",width = 170, units = "mm")
```
...
rel_abu_error_summary %>%
     ...
     dplyr::select(Metric, Pipeline, E01JH0004, E01JH0011, E01JH0016, E01JH0017, E01JH0038) %>%
      knitr::kable(booktabs = TRUE, caption = "Median and range of feature-level error rate bias (median error rate) and variance (robust COV) by pipeline and individual.", digits = 2) %>%
