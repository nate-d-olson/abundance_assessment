...
```{r}
#getting from `alpha_prop` to `alpha_bar_plot`
alpha_bar_plot <- data.frame(pipe = c("dada2", "mothur", "qiime"), 
                 p1 = c(0.6, 0.1, 0.01))
alpha_1 %>% 
   select(pipe, params_01_01) %>%
   group_by(pipe) %>%
   summarise(prop = sum(params_01_01 <= 0.05) / length(params_01_01))
   

# plotting the newly created dataframes
p <- ggplot(data = alpha_bar_plot, aes(x = pipe, y = p1, fill = pipe)) +
  geom_bar(stat = "identity") +
  geom_text(aes(label = p1), vjust = -0.3, size = 3.5) +
  theme_minimal()
p

## new plot bar function which processes data properly before making plot
plot_bar2 <- function(df, column) {
   # data processing
   ...
   ggplot(data = current_plot, aes(x = pipe, y = current_plot[[column]], fill = pipe)) +
      ...
      #add y-axis and title similar to last plot
}
```
