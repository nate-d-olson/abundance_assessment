...
test_1_1 <- readRDS("../data/mix_bayes_res/test_01_01.RDS")
test_files <- list.files("../data/mix_bayes_res/")
all_tests <- data.frame(vector("numeric", 15935))
...
alpha_1 <- select(all_tests, 1:6)
alpha_1_log <- cbind(pipe = alpha_1[, 1], -log10(alpha_1[, -1]))
beta_1 <- select(all_tests, 1, 2, 7, 12, 17, 22)
...
beta_prop <- if_else((beta_1 %>% as_tibble() %>%
                  select("params_01_01", "params_02_01", "params_10_01", "params_25_01") ) < 0.05, 
                  "p < 0.05", "p >= 0.05") %>% matrix(nrow = 15935, ncol = 5) %>% as_tibble() %>%
                  cbind(beta_1$pipe)

# re-ordering and re-naming columns 
beta_prop <- beta_prop[, c(6,1,2,3,4)]
colnames(beta_prop) <- c("pipe", "params_01_01", "params_02_01", "params_10_01", "params_25_01")
...
a1_b1_rep <- plot_bar(alpha_prop, alpha_prop_TF[["params_01_01"]], 
                 title = paste(expression(alpha), " = ", expression(beta), " = 1 (uniform)") )
