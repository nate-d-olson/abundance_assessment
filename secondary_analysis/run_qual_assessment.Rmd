...
prop_inferred <- theta_est %>%
      ## Using mothur until I have unclustered results
      filter(pipe == "mothur") %>% 
      ungroup() %>%
...
count_df <- prop_inferred %>% 
      ...
      left_join(count_df) %>% 
      mutate(t_fctr = as.numeric(t_fctr))
...
## Removing unclustered - for now ...
unmix_count_df <- unmix_count_df %>% filter(pipe != "unclustered")
...
mix_binom_bayes <- function(count_df, mix_count_df, output_file){
      ...
      mix_binom_test <- mix_count %>% 
            ...
            select(min_exp_prop, total_count, count) %>% 
            unique() %>% 
            rowwise() %>%
...
mix_count_df <- mix_count_df %>% filter(pipe != "unclustered")
count_df <- count_df %>% filter(pipe != "unclustered")
